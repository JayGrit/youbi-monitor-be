...
public class UploaderAccountRepositoryServiceImpl implements IUploaderAccountRepositoryService {
    ...
    public void ensureSchema() {
        ...
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "source_table", "VARCHAR(128) NULL");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "source_updated_at", "DATETIME NULL");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "last_upload_at", "DATETIME NULL");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "next_upload_allowed_at", "DATETIME NULL");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "upload_cooldown_min_seconds", "INT NOT NULL DEFAULT 3600");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "upload_cooldown_max_seconds", "INT NOT NULL DEFAULT 7200");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "today_upload_count", "INT NOT NULL DEFAULT 0");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "cooldown_waiting_count", "INT NOT NULL DEFAULT 0");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "upload_running_count", "INT NOT NULL DEFAULT 0");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "is_enabled", "TINYINT(1) NOT NULL DEFAULT 1");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "is_available", "TINYINT(1) NULL");
        RepositorySchemaSupport.ensureColumn(repository, TABLE, "metrics_updated_at", "DATETIME NULL");
    }
    ...
    public void refreshPlatformMetrics(String platform) {
        String normalizedPlatform = normalize(platform);
        List<String> accountKeys = repository.query(
                """
                SELECT account_key
                FROM uploader_account
                WHERE platform = ?
        ...
        for (String accountKey : accountKeys) {
            refreshMetrics(normalizedPlatform, accountKey);
        }
    }
