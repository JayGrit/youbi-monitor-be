...
import com.youbi.monitor.repository.IKuaishouAccountRepositoryService;
import com.youbi.monitor.repository.KuaishouAccountRepository;
...
@Service
public class KuaishouAccountRepositoryServiceImpl implements IKuaishouAccountRepositoryService {
    private static final String TABLE = "uploader_account_kuaishou";

    private final KuaishouAccountRepository repository;

    public KuaishouAccountRepositoryServiceImpl(KuaishouAccountRepository repository) {
    ...
    @Override
    public List<KuaishouAccountStatus> listAccounts() {
        return repository.query(
                ...
                LEFT JOIN uploader_account_kuaishou pa ON pa.topic = ua.topic
                WHERE ua.platform = 'kuaishou' AND ua.is_deprecated = 0
                ORDER BY ua.topic
                """,
                (rs, rowNum) -> {
    ...
    public void saveStorageState(String topic, String userId, String nickname, String storageState) {
        repository.update(
                """
                INSERT INTO uploader_account_kuaishou (topic, user_id, nickname, storage_state_json, updated_at)
                VALUES (?, ?, ?, ?, NOW())
