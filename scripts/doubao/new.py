...
DOUBAO_CREATOR_URL = "https://www.doubao.com/chat/"
CHROME_BIN = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
PROFILE_ROOT = Path("/private/tmp/youbi-doubao-manual-login")
DOUBAO_DOMAINS = (
    "doubao.com",
    "doubao.com.cn",
    "volces.com",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Open a fresh temporary Chrome profile, wait for Doubao scan login, then insert a new publisher_account_doubao row."
    )
    parser.add_argument("--account-key", default=os.getenv("YDBI_DOUBAO_ACCOUNT_KEY", "default"), help="Base account key. If it already exists, the script uses base-1, base-2, ...")
...
def next_account_key(args: argparse.Namespace, base_key: str) -> str:
    ...
    try:
        ...
        cursor.execute("SELECT account_key FROM publisher_account_doubao")
    ...
    for index in range(0, 1000):
        ...
        if candidate not in existing:
            return candidate
    raise RuntimeError(f"No available Doubao account key for base: {base_key}")


def is_doubao_host(value: str) -> bool:
    normalized = value.lower().lstrip(".")
    return any(normalized == domain or normalized.endswith("." + domain) for domain in DOUBAO_DOMAINS)


def filter_doubao_state(state: dict[str, Any]) -> dict[str, Any]:
    cookies = [
        cookie
        for cookie in state.get("cookies", [])
        if is_doubao_host(str(cookie.get("domain", "")))
    ]
    origins = [
        origin
        for origin in state.get("origins", [])
        if any(domain in str(origin.get("origin", "")).lower() for domain in DOUBAO_DOMAINS)
    ]
...
def page_login_hint(page) -> bool:
    ...
    negative_markers = ("扫码登录", "验证码登录", "手机登录", "密码登录", "未登录")
...
def save_storage_state(args: argparse.Namespace, storage_state_json: str, user_id: str | None, nickname: str | None) -> None:
    ...
    try:
        ...
        cursor.execute(
            """
            INSERT INTO publisher_account_doubao (account_key, user_id, nickname, storage_state_json, updated_at)
            VALUES (%s, %s, %s, %s, NOW())
            """,
...
def wait_for_login(args: argparse.Namespace) -> tuple[dict[str, Any], str, str | None, str | None]:
    ...
    print()
    print("=" * 72)
    print(f"请在弹出的 Chrome 窗口扫码登录豆包账号：account_key={args.account_key}")
...
def main() -> int:
    ...
    if cookie_count == 0 and origin_count == 0:
        raise SystemExit(f"No Doubao cookies or localStorage found after opening {final_url}.")
    ...
    print(
        "Updated publisher_account_doubao "
