...
PY_SCRIPT="${SCRIPT_DIR}/update_douyin_storage_state_from_chrome.py"
...
CHROME_DEBUG_PROFILE_DIR="${YDBI_CHROME_DEBUG_PROFILE_DIR:-${HOME}/Library/Application Support/Google/Chrome YouBi Douyin Debug}"
...
if ! curl --noproxy "*" -fsS "${CDP_URL}/json/version" >/dev/null 2>&1; then
  ...
  echo "脚本会启动一个专用的 Chrome 调试用户目录，不会占用你日常 Chrome 的默认用户目录。"
  ...
  "${CHROME_BIN}" \
    ...
    "https://creator.douyin.com/" >/tmp/youbi-douyin-chrome.log 2>&1 &
  CHROME_STARTED_BY_SCRIPT=1

  if ! wait_for_cdp 30; then
    echo "Chrome 已启动，但 9222 调试端口仍不可用。"
    echo "最近的 Chrome 日志："
    tail -n 40 /tmp/youbi-douyin-chrome.log 2>/dev/null || true
    echo
  ...
  echo "请在打开的 Chrome 里确认抖音创作中心已登录。"
  echo "如果这是第一次使用专用调试 Chrome，需要在这个窗口里重新登录一次抖音。"
  echo "如果要同步多个 Chrome Profile，请在这个调试 Chrome 里切换/打开每个 Profile，并分别登录抖音。"
  echo "所有账号登录完成后回到这个窗口按回车。"
  read "?按回车开始同步..."
fi
